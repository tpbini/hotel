...
class RB_Reservations {
    ...
    public static function create_reservation( $data ) {
        ...
        $duration_minutes = isset( $data['duration_minutes'] ) ? intval( $data['duration_minutes'] ) : ( isset( $data['duration'] ) ? intval( $data['duration'] ) : intval( get_option( 'rb_default_duration', 120 ) ) );
    ...
    public static function cancel_by_guest( $token, $reason = '' ) {
        ...
        if ( ! $res ) {
            $res = self::get_by_reference( $token );
        }
        if ( ! $res ) {
