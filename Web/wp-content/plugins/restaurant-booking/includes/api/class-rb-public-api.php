...
class RB_Public_API {
    ...
    public static function get_availability( WP_REST_Request $request ) {
        ...
        $duration = $request->get_param( 'duration' ) ? intval( $request->get_param( 'duration' ) ) : null;

        $result = RB_Availability::get_slots_for_date( $date, $party_size, $duration );
        if ( ! $result['success'] ) {
    ...
    public static function get_reservation( WP_REST_Request $request ) {
        ...
        if ( ! $res ) {
            $res = RB_Reservations::get_by_reference( $token );
        }
